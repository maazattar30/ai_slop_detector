...
def _has_auto_captions(raw: dict) -> bool:
    """Return True if video only has auto-generated captions (no manual ones)."""
    auto = raw.get("automatic_captions", {})
    subs = raw.get("subtitles", {})
    # If there are manual subtitles, it's not auto-only
    if subs:
        return False
    # Auto captions present but no manual = auto-only
    return bool(auto)


def _smart_clip(raw_path: str, out_path: str, duration: float) -> bool:
    """Clip long videos to a representative sample for analysis."""
    import subprocess, os
    # Strategy: take first 60s + middle 60s + last 60s
    # For very long videos this keeps processing time reasonable
    segment_duration = 60
    clips = []

    segments = [
        (0,                              segment_duration),           # start
        (duration / 2 - segment_duration / 2, segment_duration),     # middle
        (max(0, duration - segment_duration), segment_duration),      # end
    ]

    tmp_clips = []
    for i, (start, dur) in enumerate(segments):
        tmp = raw_path.replace("_raw.mp4", f"_clip{i}.mp4")
        cmd = [
            "ffmpeg", "-ss", str(start), "-i", raw_path,
            "-t", str(dur), "-c", "copy",
    ...
    # Write concat list
    concat_list = raw_path.replace("_raw.mp4", "_concat.txt")
